...
class RouteAnalyzer(
    ...
    private fun findChainClasses(includeLibraries: Boolean): List<ClassInfo> {
        ...
        return classes.mapNotNull { classSummary ->
            ...
            val implementsChainAction =
                classInfo.interfaces.any { iface ->
    ...
    private fun analyzeChainClassHeuristic(classInfo: ClassInfo): List<RouteInfo> {
        ...
        val allHandlers =
            (
                handlerFields.map { it.type to it.name } +
                    handlerParams.map { it.type to it.name }
            ).distinctBy { it.first }
    ...
    private fun inferHttpMethodFromName(name: String): HttpMethod {
        ...
        fun containsWord(word: String): Boolean = lowerName.contains(Regex("(^|[^a-z])$word"))
        return when {
    ...
    fun buildRouteTree(routes: List<RouteInfo>): RouteTreeNode {
        ...
        val children =
            routesByFirstSegment
                .map { (segment, segmentRoutes) ->
    ...
    private fun buildTreeNode(
        ...
        val directRoutes =
            routes.filter { route ->
                route.pathPattern
                    .removePrefix("/")
                    .split("/")
                    .size == fullPath.removePrefix("/").split("/").size
            }
        ...
        val routesByNextSegment =
            deeperRoutes.groupBy { route ->
                val segments = route.pathPattern.removePrefix("/").split("/")
    ...
    private fun generateSpringMethodSignature(route: RouteInfo): String {
        val methodName =
            route.handlerSimpleName?.let { handler ->
                handler
    ...
    private fun generateMethodNameFromPath(
        path: String,
        method: HttpMethod,
    ): String {
        val segments =
