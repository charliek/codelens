...
class TypeXrefAnalyzer(
    ...
    private fun collectSignatureRefs(
        ...
        if (info.superclass == typeFqn) out.add(ref(info, XrefKind.EXTENDS))
        if (info.interfaces.contains(typeFqn)) out.add(ref(info, XrefKind.IMPLEMENTS))

        for (field in info.fields) {
            if (baseType(field.type) == typeFqn) {
