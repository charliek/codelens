"""Formatters for class-related output."""
...
from codelens_cli.output import get_source_color

console = Console()


def print_class_list(response: ClassListResponse) -> None:
...
def print_class_detail(response: ClassDetailResponse) -> None:
...
def print_stats(stats: ScanStatistics) -> None:
...
def print_implementations(response: ImplementationsResponse) -> None:
...
def print_hierarchy(response: HierarchyResponse) -> None:
    ...
    if parent_chain:
        ...
        for i, parent in enumerate(parent_chain):
            ...
            source_color = get_source_color(parent.source.value)
    ...
    # Print current class
    source_color = get_source_color(response.hierarchy.source.value)
    ...
    if response.hierarchy.interfaces:
        ...
        for iface in response.hierarchy.interfaces:
            source_color = get_source_color(iface.source.value)
...
def _print_hierarchy_children(children: list[HierarchyNode], indent: str) -> None:
    ...
    for i, child in enumerate(children):
        ...
        source_color = get_source_color(child.source.value)
...
def print_dependencies(response: DependenciesResponse) -> None:
    ...
    if response.outgoing:
        ...
        for dep in response.outgoing:
            source_color = get_source_color(dep.source.value)
            table.add_row(
                dep.class_fqn,
